...
def apply_timeline_logic(player, events, raw_offer_dict):
    ...
    timeline_metrics["P4_Offers"] = len([t for t in filtered_offer_dict.keys() if t in P4_SCHOOLS])
    timeline_metrics["G6_Offers"] = len([t for t in filtered_offer_dict.keys() if t in G5_SCHOOLS])
    
    if filtered_offer_dict:
        formatted_offers = [f"{team} ({date})" for team, date in filtered_offer_dict.items()]
        timeline_metrics["All_Offers_List"] = ", ".join(sorted(formatted_offers))
...
class TwoFourSevenScraper:
    ...
    async def gather_all_timeline_events(self, session, player_url, player_name):
        """
        ...
        date_header_pat = re.compile(r'([A-Z][a-z]+\s+\d{1,2},\s+\d{4}):\s*([A-Za-z0-9_\-\s]+)')
        next_button_pat = re.compile(r'>Next\s*</', re.IGNORECASE)
        # Compiles all markers into a single OR (|) operation at the C-level
        footer_pat = re.compile(r'(?:CBS Sports Digital|YouTube|Facebook|X |Instagram|TikTok|Podcast)', re.IGNORECASE)

        while True:
            target_url = f"{base_timeline_url}?Page={current_page}"
            log_message(f"HTTP REQUEST FIRED TO SITE: {target_url} FOR PLAYER: {player_name}")
            
            try:
                async with session.get(target_url, cookies=self.cookies) as resp:
                    if resp.status != 200:
                        log_message(f"HTTP REQUEST FAILED WITH STATUS CODE: {resp.status}")
                        break
                    ...
                    log_message(f"READ LOG: DISCOVERED {len(matches)} HISTORICAL EVENTS ON PAGE {current_page}")
                    ...
                    for idx, match in enumerate(matches):
                        ...
                        start_pos = match.end()
                        # Limit the chunk to 500 characters max so we never grab the website footer
                        end_pos = matches[idx+1].start() if idx + 1 < len(matches) else start_pos + 500
                        chunk = html_content[start_pos:end_pos]
                        
                        # 1. Strip HTML, normalize whitespace, and hard-chop at 120 characters
                        clean_snippet = " ".join(re.sub(r'<[^>]+>', ' ', chunk).split())[:120]
                        # Strip trailing periods just in case the sentence ends with one
                      ...
                      # Ultra-Fast Contextual Tokenization (Multi-Word Mascot Support)
                        ...
                        if tag in ignore_tags:
                            ...
                            if "offer" in clean_snippet_lower:
                                prefix = clean_snippet_lower.split("offer", 1)[0]
                        ...
                        elif tag in ("commitment", "decommitment", "decommit", "enrollment", "signing"):
                            if tag in ("decommit", "decommitment"):
                                ...
                                separator = "signs letter of intent to" if "signs letter of intent to" in clean_snippet_lower else "signs with"
                            else:
                                separator = ""
                                
                            if separator and separator in clean_snippet_lower:
                                suffix = clean_snippet_lower.split(separator, 1)[-1]
                                # Safely strip broken HTML or pagination
                                suffix = suffix.split("...")[0].split("<")[0].strip()
                        ...
                        log_message(f"  READ RECORD -> DATE: {date_str} | TYPE: {event_type} | TEAM: {team_name} | SNIPPET: {clean_snippet}")
                        ...
                        if parsed_date:
                            ...
                            if event_type.lower() == "offer" and team_name != "Unknown":
                                offer_dict[team_name] = date_str

                    if next_button_pat.search(html_content):
                        log_message(f"PAGINATION SIGNATURE 'NEXT' FOUND. ADVANCING TO PAGE {current_page + 1}")
                        current_page += 1
                    else:
                        log_message("PAGINATION SIGNATURE 'NEXT' NOT FOUND. TERMINATING RECRUIT LOOP.")
                        break
                        
            except Exception as e:
                log_message(f"EXCEPTION ENCOUNTERED DURING TIMELINE PARSE LOOP: {e}")
                break
                
        return all_parsed_events, offer_dict

    async def process_historical_profile(self, session, player):
        events, raw_offer_dict = await self.gather_all_timeline_events(session, player["Profile_URL"], player["Name"])
    ...
    async def collect_year_data(self, semaphore, session, year):
        ...
        print(f"Processing structural history records for the class of {year}...")
        tasks = [self.process_historical_profile(session, p) for p in discovered_recruits]
...
async def process_batch_timelines(scraper, semaphore, session, registry):
    ...
    for player in registry:
        # This nested async function guarantees the semaphore protects every single request
        async def bounded_process(p):
            async with semaphore:
                #Cuts down the number of tasks that you have to do for debugging purposes
    ...
    log_message(f"Resuming state from local cache. Fetching timelines for {len(tasks)} players...")
    print(f"Resuming from local file. Fetching timelines for {len(tasks)} players...")
    
    return await tqdm_asyncio.gather(*tasks, desc="Timeline Extraction", unit="player")


async def main():
    # Fetch structural access tokens using the automated driver sequence
    cookies, user_agent = get_selenium_cookies("https://247sports.com/season/2022-football/recruits/")
    ...
    async with aiohttp.ClientSession(headers=scraper.headers) as session:
        ...
        if os.path.exists(OUTPUT_FILE):
            print(f"Local file '{OUTPUT_FILE}' detected. Skipping directory indexing.")
            
            # Read the existing data and replace empty Excel cells with "N/A"
            df = pd.read_excel(OUTPUT_FILE)
            df = df.fillna("N/A")
            
            # Convert the dataframe back into a list of dictionaries for the scraper
            loaded_players = df.to_dict('records')
            
            # Pass the loaded players directly to the timeline processor
            master_registry = await process_batch_timelines(scraper, semaphore, session, loaded_players)
            
        else:
            print("No local data found. Starting full directory extraction from scratch.")
            for year in range(START_YEAR, END_YEAR + 1):
                year_output = await scraper.collect_year_data(semaphore, session, year)
    ...
    print(f"Process complete. Check '{LOG_FILE}' to see exact parsing diagnostics.")

if __name__ == "__main__":
    # THE FIX: The log file is now safely cleared ONLY when running the scraper directly
